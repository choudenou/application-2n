...
def home(request,pk):
    ...
    var_interv_count = Intervention.objects.all().count()
...
def add_panne(request):
    ...
    list_logiciel=Materiel.objects.all()
    type_logiciel= Typemateriel.objects.all()
    if request.method=="POST":
        type_materiel = Typemateriel.objects.get(nom= request.POST.get('type_materiel'))
        type_logiciel = Typelogiciel.objects.get(nom= request.POST.get('type_logiciel'))
        materiel = Materiel.objects.get(nom= request.POST.get('materiel'))
        logiciel= Logiciel.objects.get(nom= request.POST.get('logiciel'))
        date_declaration_panne= request.POST.get('date_declaration_panne')
        description_panne=request.POST.get('description_panne')
        Etat_panne=request.POST.get('Etat_Panne')
        item=Panne(type_materiel=type_materiel, type_logiciel=type_logiciel,materiel=materiel, logiciel=logiciel,date_declaration_panne=date_declaration_panne, description_panne=description_panne,Etat_panne=Etat_panne)
        item.save()
        messages.info(request="panne enregistrer avec success")
    else:
        pass
    
    context = {'list_materiel':list_materiel, 'list_logiciel':list_logiciel,'type_materiel':type_materiel, 'type_logiciel':type_logiciel}  
